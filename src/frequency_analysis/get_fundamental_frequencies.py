...
class FundamentalFrequencyVsSpeedAnalysis(object):
    ...
    def __call__(self):        
        ...
        for i in tqdm(range(self.n)):
            ...
            df.loc[count] = lst
        df.to_csv('fundamental_freq_vs_Tst_analysis.csv')
        
        count = 0
        df = pd.DataFrame(columns = cols)
        self.set_Tst(1600); # Done to ensure a minimum ratio of 1/3 between Tsw and Tst
        for i in tqdm(range(self.n)):
            lst = [self.v, self.Tsw, self.Tst, self.theta]
            for j in range(8):
                ff = fe.freq_from_autocorr(self.signal[:, j+1], 1/self.dt)
            ...
            df.loc[count] = lst
        df.to_csv('fundamental_freq_vs_Tsw_analysis.csv')
